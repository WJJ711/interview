...
```shell
# 1.启动NameServer
nohup sh bin/mqnamesrv -n 47.97.197.220:9876 &  
# 2.查看启动日志
...
```shell
# 1.启动Broker
nohup sh bin/mqbroker -n 47.97.197.220:9876 -c conf/broker.conf & 
# 2.查看启动日志
tail -f logs/rocketmqlogs/broker.log 
```

3. 默认topic为TopicTest。新建自己的topic，需要用到/bin/mqadmin

   ```shell
   bin/mqadmin updateTopic -n 47.97.197.220:9876 -t stock -c DefaultCluster
   ```
...
#存储路径
storePathRootDir=/home/wjj/install/rocketmq/store
#commitLog 存储路径
storePathCommitLog=/home/wjj/install/rocketmq/store/commitlog
#消费队列存储路径存储路径
storePathConsumeQueue=/home/wjj/install/rocketmq/store/consumequeue
#消息索引存储路径
storePathIndex=/home/wjj/install/rocketmq/store/index
#checkpoint 文件存储路径
storeCheckpoint=/home/wjj/install/rocketmq/store/checkpoint
#abort 文件存储路径
abortFile=/home/wjj/install/rocketmq/store/abort
#限制的消息大小
...
#0 表示 Master，>0 表示 Slave
brokerId=0
brokerIP1=139.196.176.148
brokerIP2=139.196.176.148
#nameServer地址，分号分割
...
nohup sh bin/mqnamesrv -n 139.196.176.148:9876 &  
另一台 1核2g的
nohup sh bin/mqnamesrv -n 47.97.197.220:9876 &  
```
...
```bash
cd /usr/local/rocketmq/bin
nohup sh mqbroker -c /home/wjj/install/rocketmq/conf/2m-2s-sync/broker-a.properties &
```
...
```sh
cd /usr/local/rocketmq/bin
nohup sh mqbroker -c /home/wjj/install/rocketmq/conf/2m-2s-sync/broker-b-s.properties &
```
...
```sh
cd /usr/local/rocketmq/bin
nohup sh mqbroker -c /home/wjj/install/rocketmq/conf/2m-2s-sync/broker-b.properties &
```
...
```sh
cd /usr/local/rocketmq/bin
nohup sh mqbroker -c /home/wjj/install/rocketmq/conf/2m-2s-sync/broker-a-s.properties &
```
